...
export class CertificateListComponent implements OnInit {
  ...
  assignInstitution(certificate: Certificate) {
    ...
    const title = `Assign Institution`,
      dialogData = new AssignCertInstitutionModel(title, certificate, 'assign'),
  ...
  assignStudent(certificate: Certificate) {
    ...
    if (certificate.institutionId != null && certificate.institutionId !== 0) {
      const title = `Assign Student`,
        dialogData = new AssignCertStudentModel(title, certificate, 'assign','Save','Cancel',certificate),
  ...
  unassignStudent(certificate: Certificate) {
    this.certificateService.update(certificate.id, {accountId: 0},null).subscribe(
      (data) => {
  ...
  unassignInstitution(certificate: Certificate) {
    this.certificateService.update(certificate.id,{institutionId: 0},null).subscribe(
      data => {
